...
public class ExtensionCatalog {
    ...
    public ExtensionCatalog(List<BusinessExtension> discovered, ExtensionInstallationService installations) {
    ...
    public Optional<EventCapability> event(Long tenantId, String type) {
        ...
        return capability == null || !isEnabledForCapability(tenantId, capability) ? Optional.empty() : Optional.of(capability);
    }
    ...
    public Collection<ExtensionDescriptor> enabledDescriptors(Long tenantId) {
        return extensions.values().stream().filter(e -> installations.isEnabled(tenantId, e.descriptor().id()))
    ...
    public ExtensionDescriptor requireDescriptor(String extensionId) {
        BusinessExtension extension = extensions.get(extensionId);
        if (extension == null) throw new IllegalArgumentException("Unknown extension: " + extensionId);
        return extension.descriptor();
    }

    public void provisionNewTenant(Long tenantId) {
        extensions.values().stream().map(BusinessExtension::descriptor)
                .filter(ExtensionDescriptor::installForNewTenant)
                .filter(descriptor -> !installations.isInstalled(tenantId, descriptor.id()))
                .forEach(descriptor -> installations.install(tenantId, descriptor.id(), descriptor.version(), Map.of()));
    }
    ...
    private boolean isEnabledForCapability(Long tenantId, Object capability) {
        return extensions.values().stream().anyMatch(e -> installations.isEnabled(tenantId, e.descriptor().id())
                && (e.events().contains(capability) || e.queries().contains(capability)));
    }
