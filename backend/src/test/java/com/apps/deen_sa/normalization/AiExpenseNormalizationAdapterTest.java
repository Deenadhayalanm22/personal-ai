...
class AiExpenseNormalizationAdapterTest {
    ...
    @Test
    void recognizesGenericBankWordAndCommonTypoWithoutSwallowingNamedAccount() {
        assertThat(AiExpenseNormalizationAdapter.hasGenericBankReference(
                "paid from bank account")).isTrue();
        assertThat(AiExpenseNormalizationAdapter.hasGenericBankReference(
                "paid from bank accoujnt")).isTrue();
        assertThat(AiExpenseNormalizationAdapter.hasGenericBankReference(
                "paid from HDFC bank account")).isFalse();
    }

    @Test
    void doesNotCreateAnAccountFromAmbiguousBankTypo() {
        var properties = new ApplicationProperties(
                new ApplicationProperties.OpenAi("key", "https://api.openai.com/v1",
                        "gpt-4.1-mini", "gpt-4.1-mini", 0.55, "gpt-4o-mini-transcribe"),
                new ApplicationProperties.WhatsApp("key", "phone", "https://graph.facebook.com"));
        var references = mock(UserReferenceEntityRepository.class);
        UserReferenceEntity hdfc = new UserReferenceEntity();
        hdfc.setCanonicalName("HDFC bank account");
        UserReferenceEntity icici = new UserReferenceEntity();
        icici.setCanonicalName("ICICI bank account");
        when(references.findByUserExternalUserIdAndUserChannelAndEntityTypeAndActiveTrue(
                "9198", "WHATSAPP", UserReferenceEntityType.ACCOUNT))
                .thenReturn(List.of(hdfc, icici));
        var adapter = new AiExpenseNormalizationAdapter(mock(OpenAIClient.class), properties,
                new ExpenseTaxonomyRegistry(), references, mock(UserReferenceAliasRepository.class)) {
            @Override
            protected <T> T callAndParse(String systemPrompt, String userPrompt, Class<T> responseType) {
                assertThat(systemPrompt).contains("Ready-to-drink rose milk", "do not invent an amount split");
                return responseType.cast(new ExpenseNormalizationPort.ExpenseFacts(
                        new BigDecimal("2399"), "Shopping", "Electronics", null,
                        "bank accoujnt", LocalDate.of(2026, 9, 25), new BigDecimal("0.9")));
            }
        };

        assertThat(adapter.normalize("9198", "bought phone from bank accoujnt",
                LocalDate.of(2026, 9, 25)).sourceAccount()).isNull();
    }
}
