...
public class MfApiService {
    ...
    public MfApiService(RestTemplate http, @Value("${mfapi.base-url:https://api.mfapi.in/mf}") String baseUrl) { this.http = http; this.baseUrl = baseUrl.replaceAll("/$", ""); }
    ...
    public List<SchemeSearchResult> search(String query) {
        ...
        try {
            MfApiScheme[] result = http.getForObject(baseUrl + "/search?q={query}", MfApiScheme[].class, query.trim());
    ...
    public Optional<BigDecimal> latestNav(String schemeCode) {
        try {
            MfApiLatest result = http.getForObject(baseUrl + "/{schemeCode}/latest", MfApiLatest.class, schemeCode);
