...
A SIP requires amount, day 1–28, and start month. Creation adds an initial scheduled/due occurrence; scheduler adds a due occurrence for current month. Mutual-fund list entries expose their current SIP occurrence when one exists, including its month, status, allocation facts, and source. Confirmations require positive amount, date, a calculation source (`USER_ENTERED`, `NAV_ESTIMATED`, `STATEMENT_VERIFIED`), and NAV or units. A confirmed occurrence can be corrected from the Mutual Funds section; holdings are recalculated from confirmed transactions. Only confirmed transactions contribute to invested value/units. Current valuation/P&L are `null` without latest NAV. Errors: `409 INVESTMENT_EXISTS`, `404 INVESTMENT_NOT_FOUND`, `400 INVALID_MUTUAL_FUND`.
