...
The generic login response is intentional account-enumeration protection. The WhatsApp sign-in message includes the same one-time URL as visible text and as its button target, allowing the recipient to copy it into an external browser. `VITE_API_BASE` chooses API origin and browser requests always use `credentials: 'include'`.
