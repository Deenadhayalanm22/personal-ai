...
The story reads the current user's `monthly_financial_snapshot`, a canonical per-user/per-month projection. The snapshot currently persists commitment buckets and source evidence; it is synchronously rebuilt by loan and mutual-fund SIP writes. Future monthly financial sections and AI-ready context extend this same snapshot payload rather than introducing another commitment calculation or table.
