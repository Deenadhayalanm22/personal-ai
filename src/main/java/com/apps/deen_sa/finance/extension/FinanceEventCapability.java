...
final class FinanceEventCapability implements EventCapability {
    ...
    FinanceEventCapability(String eventType, SpeechHandler delegate, TransactionTemplate transactions) {
    ...
    @Override public java.util.Map<String, String> fieldTypes() {
        ...
        values.put("creditCardBillingDay", "integer"); values.put("creditCardDueDay", "integer"); values.put("tags", "array");
    ...
    public CapabilityResult handle(ExtensionEvent extensionEvent, String rawText, CapabilityContext capabilityContext,
        ...
        SpeechResult result = transactions.execute(status -> execute(event, rawText, context, continuation));
        return toCapabilityResult(result);
    }

    private SpeechResult execute(EventPatch event, String rawText, ConversationContext context, boolean continuation) {
        SpeechResult result;
